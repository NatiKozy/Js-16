...
function makeOne() {
	...
	paragraph.textContent = allForms.length;
}
...
function makeThree() {
	...
	paragraphThree.textContent = name;
}
...
function makeFour() {
	...
	for (let el of allForms) {
		const elName = el.getAttribute('name');
		formNames.push(elName);
	}
	const joinformNames = formNames.join(', ');
	paragraphFour.textContent = joinformNames;
}
...
function makeFive() {
	...
	const countEl = formThree.elements;
	paragraphFive.textContent = countEl.length;
}
...
function makeSix() {
	...
	const countEl = formTwo.elements;
	paragraphSix.textContent = countEl.length;
}
...
function makeEight() {
	//Ваш код
	const formOne = document.forms.formOne;
	const countEl = formOne.elements;
	const names = [];
	for (let el of countEl) {
		const elName = el.getAttribute('name');
		names.push(elName);
	...
	paragraphEight.textContent = joinNames;
}
...
function makeNine() {
	...
	for (let el of countEl) {
		const elName = el.getAttribute('name');
		namesThree.push(elName);
	}
	const joinNamesThree = namesThree.join(', ');
	paragraphNine.textContent = joinNamesThree;
}
...
function makeTen() {
	//Ваш код
	const lastFormEl = document.forms.lastForm.elements;
	paragraphTen.textContent = lastFormEl.fourthName.value;
}
...
function makeEleven() {
	...
	paragraphEleven.textContent = optionsValue;
}
...
formOne.addEventListener('submit', function (event) {
	...
	const nameInput = formOne.elements.firstName;
	const emailInput = formOne.elements.firstEmail;
	if (nameInput.value == '' || emailInput.value == '') {
		error.textContent = 'ошибка, заполните все поля'
	} else {
		formOne.submit();
	}
	//Ваш код
...
document.querySelector('.b-26').onclick = function (event) {
	...
	//Ваш код
	const nameInput = document.forms.formThree.elements.thirdName;
	document.getElementById('result26').textContent = nameInput.getAttribute('placeholder');
